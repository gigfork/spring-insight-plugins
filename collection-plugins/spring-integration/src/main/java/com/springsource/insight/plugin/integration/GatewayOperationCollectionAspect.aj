...
public privileged aspect GatewayOperationCollectionAspect extends AbstractOperationCollectionAspect {
    public static final OperationType TYPE = OperationType.valueOf("integration_gateway_operation");
    ...
    public pointcut collectionPoint() : 
        execution(protected void org.springframework.integration.gateway.MessagingGatewaySupport+.send(Object)) ||
        execution(protected Object org.springframework.integration.gateway.MessagingGatewaySupport+.receive()) ||
        execution(protected Object org.springframework.integration.gateway.MessagingGatewaySupport+.sendAndReceive(Object)) ||
        execution(protected Message org.springframework.integration.gateway.MessagingGatewaySupport+.sendAndReceiveMessage(Object));
    ...
    private static void fillOperation(JoinPoint jp, Operation op) {
        ...
        if (ArrayUtil.length(args) > 0) {
            ...
            if (obj instanceof Message) {
                payloadObj = ((Message)obj).getPayload();
            ...
            if (payloadClass != null) {
                op.put("payloadType", payloadClass.getName());
            }
        }
        
        op.label(label)
          .put("siComponentType", "Gateway")
          .put("siSpecificType", gateway.getComponentName())
